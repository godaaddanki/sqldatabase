...
namespace PeterDBTesting {
    ...
    TEST_F(RM_Private_Test, scan_after_update) {
        ...
        for (int i = 0; i < numTuples; i = i + 100) {
            ...
            updateCount++;
            if (i % 10000 == 0) {
                GTEST_LOG_(INFO) << updateCount << "/" << numTuples << " records have been updated so far."
                                 << std::endl;
            }
        }
    ...
    TEST_F(RM_Private_Test, create_table_with_same_name) {
        std::vector<PeterDB::Attribute> table_attrs = parseDDL("CREATE TABLE " + tableName +
    ...
    TEST_F(RM_Version_Test, extra_multiple_add_drop_mix) {
        ...
        ASSERT_EQ(rm.printTuple(attrs2, outBuffer, stream), success) << "RelationManager::printTuple() should succeed.";
        ...
        stream.clear();
        ASSERT_EQ(rm.printTuple(attrs, outBuffer, stream), success) << "RelationManager::printTuple() should succeed.";

        checkPrintRecord("emp_name: Peter Anteater, height: 185.7, salary: NULL", stream.str());
